...
class Posts(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='Пост_Пользователя')
...
class StatusFilterResponses(models.Manager):
    def get_queryset(self):
        ...
        return super().get_queryset().filter(Q(status=False, user=get_current_user()) |
                                             Q(status=False, post__user__username=get_current_user()) |
                                             Q(status=True))
...
class Responses(models.Model):
    ...
    post = models.ForeignKey('Posts', on_delete=models.CASCADE, verbose_name='Объявление', related_name='responses_posts')
